...
# ---- Nutrition summary for compare view ----------------------------------
def _nutrition_summary(product: dict):
    """Return dict with cal, protein, fat, carb, sug (prefer per serving, fallback to per 100g)."""
    n = product.get("nutriments", {}) if product else {}
    def first(*keys):
        for k in keys:
            v = n.get(k)
            if v is not None:
    ...
    # calories: kcal preferred; convert kJ if needed
    kcal_srv = first("energy-kcal_serving")
    if kcal_srv is None:
        kj = first("energy_serving")
        kcal_srv = (kj/4.184) if kj is not None else None

    kcal_100 = first("energy-kcal_100g")
    if kcal_100 is None:
        kj = first("energy_100g")
        kcal_100 = (kj/4.184) if kj is not None else None

    def pick(srv, per100):  # use serving when available; else per 100g
        return srv if srv is not None else per100

    return {
        "cal":    pick(kcal_srv, kcal_100),
        "protein":pick(first("proteins_serving"),       first("proteins_100g")),
        "fat":    pick(first("fat_serving"),            first("fat_100g")),
        "carb":   pick(first("carbohydrates_serving"),  first("carbohydrates_100g")),
        "sug":    pick(first("sugars_serving"),         first("sugars_100g")),
    }
    ...
    if not compare_two:
        ...
        def _process_compare(src, tag):
            ...
            if prod:
                ...
                summary = _nutrition_summary(prod)
                best_img = prod.get("image_front_best") or prod.get("image_url")
                return {"name": prod["name"], "label": label, "summary": summary, "image": best_img, "product": prod}
            else:
